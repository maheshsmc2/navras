...
async function loadOttPosterGrid(platform) {
  ...
  const films = (ottFilms[platform] || []).slice(0, 10);

  const withData = await Promise.all(films.map(async f => {
    try {
      const data = await TMDB.get(`/movie/${f.tmdbId}`, {});
      return { f, film: data?.success === false ? null : data };
    } catch { return { f, film: null }; }
  }));

  grid.innerHTML = withData.map(({ f, film }) => renderOttCard(f, film)).join('');
}
